	SUBROUTINE NESTED_PIXNO(EE,IRES,IPIX)
C
	REAL*4 EE(3),E(3)	! input unit vector
	INTEGER IRES		! resolution
	INTEGER IPIX		! pixel number in `nested' scheme
C
...
C	need E to be normalized
	EM = EE(1)**2 + EE(2)**2 + EE(3)**2
	IF(ABS(EM-1).GT.5.E-3) THEN
	  F = 1./SQRT(EM)
	ELSE
	  F = 8./(3.+EM*(6.-EM))
	ENDIF
...
C	write (*,*) xp,yp
	...
	IPIX = NFACE*IT28(IRES+1) + NINFACE
	RETURN
	END
