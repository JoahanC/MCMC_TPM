C	NEOCAM-steep-MCMC-PJDFC.f
C	gfortran -o NEOCAM-steep-MCMC-PJDFC NEOCAM-steep-MCMC-PJDFC.f READ_TABLE-steep.f ~/Ned.a
C
...
C	modified 25-MAY-2017 to do NEOCAM instead of WISE
...
C
	PARAMETER (NP=8,NDMX=99,NB=2)		! change for NEOCAM
	REAL*4 PHT(NDMX),DET(NDMX),RAT(NDMX),DECT(NDMX),DST(NDMX)
	REAL*4 SIGT(NB,NDMX),MAGT(NB,NDMX)	! change for NEOCAM
	REAL*8 T0(NDMX)
	COMMON /NRMLPTS/ T0,H,DH,PER,MAGT,SIGT,DST,DET,PHT,RAT,DECT,NDM
	COMMON /PREDICTED/ WMAGT(NB,NDMX)	! change for NEOCAM
	REAL*8 MJD,JD(NDMX)
	REAL*8 T1JAN2010,T,JD1JAN2010
	REAL*8 TSTART,TWALL
	REAL*8 JULIAN_DAY,GMT2T68
	REAL*4 EA(3),A(3),E(3)
	REAL*4 WMAG(NB),SIG(NB)			! change for NEOCAM
	REAL*4 P(NP)
...
C
	REAL*4 SIGMIN(NB) /NB*0.02/		! change for NEOCAM
	EXTERNAL RAN
C
...
C
	...
	WRITE (*,FMT='(A)') 'Enter MJD,RA,Dec,Delta,NC12 with sigmas:'	! change for NEOCAM
	WRITE (*,FMT='(A)') '   MJD      RA      DEC     DE      DS'//
	1	'    PHASE NC12 & sigmas'				! change for NEOCAM
10	READ (*,900,END=100) MJD,RA,DEC,DELTA,(WMAG(I),SIG(I),I=1,NB)	! change for NEOCAM
900	FORMAT(F11.5,2F11.7,F8.4,4(F7.3,F6.3))
	IF (NDM.GE.NDMX) STOP '2 much data'
	NDM=NDM+1
	DO I=1,NB							! change for NEOCAM
	  MAGT(I,NDM) = WMAG(I)
...
C	CALL EPHEM(-1,T0(NDM),E)					! change for NEOCAM
	CALL NEOCAM_ORBIT(T0(NDM),E)					! change for NEOCAM
	CALL RADEC_TO_C(RAT(NDM)/DTR,DECT(NDM)/DTR,EA)
	DO J=1,3
C	  E(J)=-E(J)							! change for NEOCAM
	  EA(J) = EA(J)*DET(NDM)
	  A(J) = EA(J)+E(J)
	ENDDO
	...
	1	PHT(NDM),(WMAG(I),SIG(I),I=1,NB)			! change for NEOCAM
...
C	also apj408731t1_mrt.txt from Mainzer etal (2011) for mean 0.563
...
C	Z**(NP-2) was Z**4 until 21-DEC-2017
	1	Q = MIN(1.0,ABS(SZTH/SIN(TH))*Z**(NP-2)*EXP(XP))
	    U = RAN(IR,JR)
	    IF (U.LT.Q) THEN
C	accept
C
...
C
	PARAMETER (NDMX=99)
	PARAMETER (NBAND=2,NL=6)			! change for NEOCAM
	REAL*4 PHT(NDMX),DET(NDMX),RAT(NDMX),DECT(NDMX),DST(NDMX)
	REAL*4 SIGT(NBAND,NDMX),MAGT(NBAND,NDMX)	! change for NEOCAM
	REAL*8 T0(NDMX)
	COMMON /NRMLPTS/ T0,H,DH,PER,MAGT,SIGT,DST,DET,PHT,RAT,DECT,NDM
	COMMON /PREDICTED/ WMAGT(NBAND,NDMX)		! change for NEOCAM
...
C
C	block of changes for NEOCAM
C	REAL*4 WMAG(NBAND)
C	REAL*4 LAM(NL) /3.0974,3.6298,5.0450,4.4130,14.6388,10.0348,
C	1	20.4156,23.5869/
C	REAL*4 W(NL) /0.51167,0.47952,0.3165,0.6778,0.3775,0.5188,
C	1	0.4554,0.5351/
C	REAL*4 WVL0(NBAND) /3.3526,4.6028,11.5608,22.0883/
C	REAL*4 F0(NBAND) /306.681,170.663,29.0448,8.2839/
	REAL*4 WMAG(NBAND)
...
C	prior should be ln(PIR2PV)=0.563+/-0.340, see apj498_786_t1_mrt.f, also apj408731t1_mrt.txt
	PIR2PV = EXP(P(8))
...
C	write (*,fmt='(3f8.4,f10.4,1pe10.3)') pole,alb,theta1
	DO J=1,NDM
C	  CALL EPHEM(-1,T0(J),R)		! change for NEOCAM
C	  DO I=1,3				! change for NEOCAM
C	    R(I) = -R(I)			! change for NEOCAM
C	  ENDDO					! change for NEOCAM
	  CALL NEOCAM_ORBIT(T0(J),R)		! change for NEOCAM
...
C	block of changes for NEOCAM
C	  DO I=1,NBAND
C	    WMAGT(I,J)=-2.5*LOG10((W(2*I-1)*F(2*I-1)+W(2*I)*F(2*I))
C	1	/F0(I))
C	  ENDDO
...
C       the solar flux, nu*F_\nu, in erg/cm^2/sec/oct.  the 1.056 is a fudge factor
C	to make the V and W1 fluxes right
