        ...
        if d_max <= 0:
            ...
            for i in range(n):
                ...
                for j in range(len_measurements):
                    E = abs((measurements[j] - xc - newdx[i]) / sigmas[j])
                    if E > emx:
                        CS = 2 * emx * E - emx ** 2
                    else:
                        CS = E ** 2
...
if len(sys.argv) < 2:
    print("Missing packed designation!")
    print("Run file with the following format without brackets:\n")
    print("python tbl2MCMCin.py [packed designation]")
    exit()
...
print("gfortran -o ../WISE-steep-rc-MCMC-PJDFCS WISE-steep-rc-MCMC-PJDFCS.f Nedlib.a")
output_file.write("gfortran -o ../WISE-steep-rc-MCMC-PJDFCS WISE-steep-rc-MCMC-PJDFCS.f Nedlib.a\n")
...
print("../WISE-steep-rc-MCMC-PJDFCS << LAST")
output_file.write("../WISE-steep-rc-MCMC-PJDFCS << LAST\n")
...
for inline in files:
    ...
    for epoch in epochs:
        ...
        obs_dist = get_distance(horizon_name, mid_time)

        oline = "%9.3f,%7.3f,%+7.3f,%7.5f"%(mid_time, mid_ra, mid_dec, obs_dist)

        for band in ["w1", "w2", "w3", "w4"]:
            if band + "flux" in epoch_data.columns and "_3b" not in inline:

                meas_masked = epoch_data[band + "flux"]
                sig_masked = epoch_data[band + "sigflux"]
                ...
                meanmag = z_p[band] - 2.5*numpy.log10(abs(meanflux))
                ...
                meas_masked = epoch_data[band + "mpro"]
                sig_masked = epoch_data[band + "sigmpro"]
