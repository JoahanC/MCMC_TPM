C	Herschel-WISE-rc-MCMC-1234.f
C	g77 -o Herschel-WISE-rc-MCMC-1234 Herschel-WISE-rc-MCMC-1234.f ~/Ned.a
C
...
C
C	any point with SIGMA_M > 8.99 is ignored entirely
C
C	first line is a Herschel PACS epoch with 70, 100 & 160 micron fluxes in mJy
C
	PARAMETER (NDMX=9, NBAND=9)
	REAL*4 PHT(NDMX),DET(NDMX),RAT(NDMX),DECT(NDMX),DST(NDMX)
	REAL*4 SIGT(NBAND,NDMX),MAGT(NBAND,NDMX)
	REAL*4 XYZ(3,NDMX)
	REAL*8 T0(NDMX)
	COMMON /NRMLPTS/ T0,H,XYZ,MAGT,DH,SIGT,DST,DET,PHT,RAT,DECT,NDM
	...
	REAL*4 EA(3),A(3),E(3)
	REAL*4 WMAG(4),SIG(4),HFLUX(3),HSIG(3)
	REAL*4 P(4)
...
C
	...
	DTR = 45/ATAN(1.)
C	read the Herschel point
	NDM = 1
	WRITE (*,FMT='(A)') 'Enter RA,Dec,distance of Herschel from Sun:'
	READ (*,900) RAH,DECH,DIST
	CALL RADEC_TO_C(RAH/DTR,DECH/DTR,XYZ(1,NDM))
	DO J=1,3
	  XYZ(J,NDM) = DIST*XYZ(J,NDM)
	ENDDO
	WRITE (*,FMT='(A)') 'Enter MJD,RA,Dec,Delta,F70-100-160 with sigmas:'
	READ (*,900) MJD,RA,DEC,DELTA,(HFLUX(I),HSIG(I),I=1,3)
	T0(NDM) = 86400*(MJD+2400000.5D0-JD1JAN2010)+T1JAN2010
	RAT(NDM) = RA
	DECT(NDM) = DEC
	CALL RADEC_TO_C(RA/DTR,DEC/DTR,EA)
	DO J=1,3
	  EA(J) = DELTA*EA(J)
	  A(J) = EA(J)+XYZ(J,NDM)
	ENDDO
...
C	no WISE data for Herschel point
	DO I=1,6
	  MAGT(I,NDM) = 10.
	  SIGT(I,NDM) = 9.99
	ENDDO
	DO I=1,3
C	use AB scale for Herschel
	  MAGT(I+6,NDM) = 2.5*LOG10(3630./ABS(HFLUX(I)))
	  SIGT(I+6,NDM) = 2.5*LOG10(1.+HSIG(I)/ABS(HFLUX(I)))
	  IF (HFLUX(I).LT.0.) SIGT(I+6,NDM) = -SIGT(I+6,NDM)
	ENDDO
C
	WRITE (*,FMT='(A)') 'Enter MJD,RA,Dec,Delta,W1234 with sigmas:'
	WRITE (*,FMT='(A)') '   MJD      RA      DEC     DE      DS    PHASE W1234 & sigmas'
...
900	FORMAT(F11.5,2F11.7,F8.4,4(F7.3,F6.3))
	...
	ENDDO
C	copy bands 3 & 4 for the W3p and W4p weights
	DO I=3,4
	  MAGT(I+2,NDM) = MAGT(I,NDM)
	  SIGT(I+2,NDM) = 9.99
	ENDDO
C	no Herschel data for WISE
	DO I=7,9
	  MAGT(I,NDM) = 10.
	  SIGT(I,NDM) = 9.99
	ENDDO
	write (*,fmt='(I1,18F7.2)') NDM,(MAGT(I,NDM),SIGT(I,NDM),I=1,NBAND)
	RAT(NDM) = RA
	DECT(NDM) = DEC
	DET(NDM) = DELTA
	T0(NDM) = 86400*(MJD+2400000.5D0-JD1JAN2010)+T1JAN2010
	CALL EPHEM(-1,T0(NDM),XYZ(1,NDM))
...
100	WRITE (*,*)
	DO I=1,NDM
	  WRITE (*,FMT='(2F8.3,2F8.4,F6.2,9(F7.2,F6.2))') 
	1	RAT(I),DECT(I),DET(I),DST(I),PHT(I),(MAGT(J,I),SIGT(J,I),J=1,NBAND)
	ENDDO
	WRITE (*,*) NDM
...
C	accept
	      ...
	      PPMIL = 0.5+1000*SDIAM(KW)/DIAM(KW)
	      WRITE (21,FMT='(F7.4,3F8.4,F11.5,F12.3,2I5)') 
	1	(PW(I,KW),I=1,4),DIAM(KW),CH2(KW),NTRY(KW),PPMIL
	      CLOSE (UNIT=21)
...
C
	...
	REAL*4 WMAG(NBAND)
	INTEGER ML(NBAND) /2,2,3,2,3,2,2,2,2/
	REAL*4 LAM(NL) /3.0974,3.6298,5.0450,4.4130,
	1	8.0145,11.495,15.256,21.150,24.690,
	2	7.9898,11.2191,15.1258,21.2840,24.8272,
	3	66.2077,82.1308,92.9428,115.9681,146.8949,198.5176/
	REAL*4 W(NL) /0.51167,0.47952,0.3165,0.6778,
	1	0.17647,0.49203,0.24554,0.69427,0.30588,
	2	0.2052,0.48726,0.22026,0.6788,0.31141,
	3	0.73831,0.25713,0.69427,0.30588,0.76785,0.23780/
	REAL*4 WVL0(NBAND) /3.3526,4.6028,11.5608,22.0883,
	1	11.1678,22.3425,70.,100.,160./
	PARAMETER (NLT=119)
	REAL*4 WVLT(NLT)
	...
	REAL*4 F0(NBAND) /306.681,170.663,29.0448,8.2839,31.066,8.0924,3*3630./
	REAL*4 SIGMIN(NBAND) /9*0.01/
	REAL*4 MICRONS,NUFNU
C	parameters of albedo prior
	PARAMETER (PK1=0.03,FD=0.251,PK2=0.167)
C
	DO I=1,NLT
	  WVLT(I) = 0.55*(28./0.55)**((I-1)/79.)
	ENDDO
...
C	write (*,fmt='(3f8.4,f10.4,1pe10.3)') pole,alb,theta1
	DO J=1,NDM
	  ...
	  CALL PREDICT(XYZ(1,J),N,DET(J),POLE,THETA,ALB,EM,ASYM,ROT,NALL,WVALL,F)
	  DO I=1,NALL
	    FALL(I,J) = F(I)
	  ENDDO
	  KH=0
...
C	do fit for best diameter
	...
	IF (DH.LT.2.) THEN
	  NP = NP+1
C	compute ln(D) and sigma(ln(D)) for pre 12-Aug-2014:
C	  DIAM(NP) = ALOG(1329./10.**(0.2*H)/SQRT(PV))
C	  SDIAM(NP) = LN10OVER5*DH
C	compute D^2 and sigma(D^2) for post 12-Aug-2014:
	  AREA(NP) = (1329./10.**(0.2*H)/SQRT(PV))**2
	  SAREA(NP) = (10.**(0.4*DH)-1)*AREA(NP)
	ENDIF
	DO J=1,NDM
	  DO I=1,NBAND
	    IF (SIGT(I,J).LT.8.99) THEN
	      NP = NP+1
C	compute ln(D) and sigma(ln(D)) for pre 12-Aug-2014:
C	      DIAM(NP) = ALOG(10.**(0.2*(WMAGT(I,J)-MAGT(I,J))))
C	      SDIAM(NP) = LN10OVER5*ABS(SIGT(I,J))
C	compute D^2 and sigma(D^2) for post 12-Aug-2014:
	      AREA(NP) = 10.**(0.4*(WMAGT(I,J)-MAGT(I,J)))
	      SAREA(NP) = (10.**(0.4*ABS(SIGT(I,J)))-1)*AREA(NP)
	      IF (SIGT(I,J).LT.0.) AREA(NP) = -AREA(NP)
	    ENDIF
	  ENDDO
	ENDDO
C
C	code pre 12-AUG-2014:
C	CALL ROBUST(NP,DIAM,SDIAM,LND,SLND,RCS)
C	DIA = EXP(LND)
C	SDIA = DIA*SLND
C	code for using linear fluxes post 12-AUG-2014:
	CALL ROBUST(NP,AREA,SAREA,DSQ,SDSQ,RCS)
	if (dsq.lt.0.) stop 'DSQ < 0'
	DIA = SQRT(DSQ)
	SDIA = 0.5*DIA*SDSQ/DSQ
C
	DM = -5.*ALOG10(DIA)
C
	DO J=1,NDM
...
C
	IF (ERROR.LT.BESTEVER) THEN
	  ...
	  DO I=1,4
	    PB(I) = P(I)
	  ENDDO
	  WRITE (22,FMT='(1H%,F11.6,3F12.6,F12.4,F9.4)') P,ERROR,RCS
	  WRITE (22,FMT='(1H%,F10.5,1P2E11.4)') PV,DIA,THETA1
	  DO J=1,NDM
	    WRITE (22,FMT='(1H%,9F8.3)') (WMAGT(I,J),I=1,NBAND)
	  ENDDO
	  WRITE (22,FMT='(A)') '/wvl ['
	  WRITE (22,FMT='(10F8.3)') (WVALL(I+NL+NBAND),I=1,NLT)
	  WRITE (22,FMT='(A)') '] def'
	  DO J=1,NDM
C	set up colors for different epochs
	    IF (MOD(J,4).EQ.1.AND.NDM.GT.1) WRITE (22,FMT='(A)') '1 0 0 SRGB'
	    IF (MOD(J,4).EQ.2) THEN
	      IF (NDM.GT.2) THEN
	        WRITE (22,FMT='(A)') '0 0.7 0 SRGB'
	      ELSE
	        WRITE (22,FMT='(A)') '0 0 1 SRGB'
	      ENDIF
	    ENDIF
...
C
	    DO I=1,NBAND
	      ...
	      IF (SIGT(I,J).LT.9.00)
	1       WRITE (22,FMT='(2F7.3,1PE10.3,3H QQ)') MICRONS,SIGT(I,J),NUFNU
	    ENDDO
	    WRITE (22,FMT='(A)') '/ft ['
	    DO I=1,NLT
...
C
C	NP input points in X with sigmas's in SG
...
C	(PI*R**2)*FSUN/(PI*AU**2) = FSUN*(R/(DE*AU))**2
	  PV = ALBEDO/Q
	  PIR=PV*EXP(MAX(0.,MIN(1.,LOG(LAM(ILAM)/0.55)/LOG(3.4/0.55)))*LOG(PIR2PV))
	  FHG = PHI*PIR*FSUN*(5.E4/AU)**2
