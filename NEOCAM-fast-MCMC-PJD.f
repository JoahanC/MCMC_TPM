C	NEOCAM-fast-MCMC-PJD.f
C	gfortran -o NEOCAM-fast-MCMC-PJD NEOCAM-fast-MCMC-PJD.f ~/Ned.a
C
...
C
C	should terminate after writing 16200 with 24300 total
C	should write to fort.21 after 300, 900, 2700 ... 24300 accepted steps
C	should burn the first 1/3 of the acceptances
C	no writes to fort.23
C
C	READ_TABLE will read the tables made by make-tables.f
C
...
C	guess diameter from 100 meter to 10 km
	  IF (I.EQ.1) THEN
	    PW(6,I) = ALOG(.1)+ALOG(1.E2)*RAN(IR,JR)
	  ELSE
	    PW(6,I) = PW(6,1)+GAUSRN(IR,JR)
	  ENDIF
	  DO J=1,6
	    P(J) = PW(J,I)
	  ENDDO
...
C	    WRITE (*,FMT='(F8.5,3F9.5)') Z,P3
	    ...
	    IMC = IMC+1
	    IF (MOD(IMC,1000).EQ.0) WRITE (*,*) IMC,TWALL
	    Q = 0
	    XP = -0.5*(CHSQ-CH2(KW))
	    IF (XP.GT.50.) Q = 1
	    IF (ABS(XP).LT.50.) 
...
C
C	was:
C	      OPEN (UNIT=21,ACCESS='APPEND')
C	write out the old position
C	      WRITE (21,FMT='(F8.5,5F9.5,F17.5,I5)') 
C	1	(PW(I,KW),I=1,NP),CH2(KW),NTRY(KW)
C	      CLOSE (UNIT=21)
C	      WRITE (*,FMT='(F7.4,5F8.4,F17.5,I4,I3)') 
C	1	(PW(I,KW),I=1,NP),CH2(KW),NTRY(KW),(KW-1)
...
C	reject: just increment the weight
	      NTRY(KW) = NTRY(KW)+1
C	      WRITE (*,FMT='(F7.4,5F8.4,2F11.3,I3)') 
C	1	P,CHSQ,CH2(KW),(KW-1)
	    ENDIF
	  ENDDO
	GO TO 200
	END
...
C	T1 = 401 K
	...
	IF (THETA1.LE.0) WRITE (*,FMT='(A,7F8.4)') 
	1	'Theta1 <= 0:',THETA1,P
...
C	write (*,fmt='(3f8.4,f10.4,1pe10.3)') pole,alb,theta1
	DO J=1,NDM
	  ...
	  DO I=1,3
	    R(I) = -R(I)
	  ENDDO
	  IF (THETA.LT.0.) WRITE (*,*) 'Theta < 0:',THETA
...
C	penalize large P(3) which ln(albedo)
	IF (P(3).GT.0.7) ERROR = ERROR+100*(P(3)-0.7)**2
...
C	discourage periods longer than 1745 hours
	  IF (P(4).GT.7.4645) ERROR=ERROR+100*(P(4)-7.4645)**2
C	and periods less than 3.6 seconds
	  IF (P(4).LT.-6.9) ERROR=ERROR+100*(P(4)+6.9)**2
	ELSE
...
C
C	now:
	IF (THRM.LT.0.) WRITE (*,*) 'THRM < 0:',THRM
...
C	extrapolate from longer wavelengths if needed
	  IF (I.GE.3 .AND. TBRITE(I).LE.0.) 
	1	TBRITE(I) = 2*TBRITE(I-1)-TBRITE(I-2)
C	minimum from READ_TABLES, pretty cold
	  TBRITE(I) = MAX(1.92661153E-03,TBRITE(I))
	  X = X+X
	ENDDO
	RETURN
	END
...
3     XMIN=X
	...
	REAL INSOLT(NTIME,NFACE,NLAT,NSL)		! solar input
	REAL TT(NTIME,NFACE,NLAT,NTHRM,NSL)	! the temperatures
C
	COMMON /TABLES/ INSOLT,TT
	COMMON /TBLMT/ SOLMIN,SOLMAX,TMIN,TMAX
C
	OPEN (UNIT=41,FILE='/Users/wright/asteroid/rc-tables.dat',
	1	FORM='unformatted')
	DO ISL=1,NSL
	  READ (41) (((INSOLT(I,J,K,ISL),I=1,NTIME),J=1,NFACE),K=1,NLAT)
	  DO IT=1,NTHRM
	    READ (41) (((TT(I,J,K,IT,ISL),I=1,NTIME),J=1,NFACE),K=1,NLAT)
	  ENDDO
	ENDDO
	CLOSE (UNIT=41)
C
...
C
	...
	PARAMETER (NFACE=127,NLAT=16,NTIME=32,NSL=31,NTHRM=46)
	REAL INSOL(NTIME,NFACE,NLAT), T(NTIME,NFACE,NLAT)
C
	REAL INSOLT(NTIME,NFACE,NLAT,NSL)		! solar input
	REAL TT(NTIME,NFACE,NLAT,NTHRM,NSL)	! the temperatures
C
...
C
	...
	1	FSL*INSOLT(I,J,K,ISL+1)
	      IF (INSOL(I,J,K).LT.SOLMIN) INSOL(I,J,K)=SOLMIN
	      IF (INSOL(I,J,K).GT.SOLMAX) INSOL(I,J,K)=SOLMAX
	      T(I,J,K) = 
	1	(1-FSL)*((1-FX)*TT(I,J,K,IX,ISL)+FX*TT(I,J,K,IX+1,ISL))
	2	+FSL*((1-FX)*TT(I,J,K,IX,ISL+1)+FX*TT(I,J,K,IX+1,ISL+1))
	      IF (T(I,J,K).LT.TMIN) T(I,J,K)=TMIN
	      IF (T(I,J,K).GT.TMAX) T(I,J,K)=TMAX
	    ENDDO
	  ENDDO
	ENDDO
	RETURN
