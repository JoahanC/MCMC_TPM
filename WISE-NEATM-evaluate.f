C	WISE-NEATM-evaluate.f
C	gfortran -o WISE-NEATM-evaluate WISE-NEATM-evaluate.f ~/Ned.a
C
...
C	period is just ignored.  Included for compatibility
	...
	WRITE (*,FMT='(A,$)') 
	1	'Enter ln(p_V), ln(p_IR/p_V), ln(eta), ln(dia) in km:'
	READ (*,FMT='(4F10.4)') P
	PV = EXP(P(1))
	PVMAX = 1.5
	P(1) = LOG(PV*PVMAX/(PVMAX-PV))
