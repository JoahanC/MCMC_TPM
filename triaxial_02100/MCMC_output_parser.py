...
def retrieve_output_data():
    ...
    period_file = open("DvsPeriod.dat", 'r')
    ...
    print('min', (min(diameters)))
    count = 0
    exempt = [6608, 25932, 41328]
    for line in output_file.readlines():
        print(count)
        print(round(np.e ** float(line.strip()[44:52].strip()), len(str(diameters[count])) - 2), diameters[count])
        ...
        #print(diameter, diameters[count])
    ...
    for line in period_file.readlines():
        periods.append(float(line.strip().split()[1]))

    
    print('test', len(diameters_true))
    output_file.close()
    period_file.close()
    albedo_file.close()


    return diameters_true, albedos, gammas, chis, periods
...
os.system("/bin/cp fort.21 PJDFCS.out")
os.system(f"echo 'PJDFCS.out' | ../read-WISE-rc-MCMC-PJDFCS --> best_fit.txt") 
